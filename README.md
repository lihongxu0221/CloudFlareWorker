...
使用本地 Wrangler / 一键脚本前，先从模板生成配置文件（`wrangler.toml` 已加入 `.gitignore`，勿提交）：
...
然后编辑 `wrangler.toml` 中的 `DOMAIN` 为你的真实域名。一键脚本若发现本地没有 `wrangler.toml`，也会自动从 `wrangler.toml.example` 复制。

### 方式一：Cloudflare 控制台（推荐，零本地环境）

1. 打开 [Cloudflare Dashboard](https://dash.cloudflare.com) → **Workers & Pages** → **Create** → **Create Worker**
2. 给 Worker 起名（如 `grokreg-mail`），点击 **Deploy**
...
# 2. 从模板生成本地配置（Windows 用 Copy-Item，见上方）
cp wrangler.toml.example wrangler.toml
# 编辑 wrangler.toml 中的 DOMAIN
...
├── wrangler.toml.example     # Wrangler 配置模板（复制为 wrangler.toml）
├── wrangler.toml             # 本地部署配置（gitignore，勿提交）
